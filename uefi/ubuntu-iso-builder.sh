...
cp ubuntu-server-rapiscan.seed $WORK/preseed/
cp grub.cfg $WORK/boot/grub/

# magic mkiso incantation
mkisofs -D -r -V “AUTOINSTALL” -cache-inodes -J -l -b isolinux/isolinux.bin -c isolinux/boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -eltorito-alt-boot -b boot/grub/efi.img -no-emul-boot -o $OUTPUT $WORK
echo "========================================================================"
